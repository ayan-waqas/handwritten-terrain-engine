...
void main() {
    ...
    vec3 hemiAmbient = mix(groundAmbient, skyAmbient, upWeight) * 0.45 * dayFactor;
    
    float shadow = calculateShadow(FragPosLightSpace, norm, lightDirection);
    float diff = max(dot(norm, lightDirection), 0.0);
    vec3 diffuse = (1.0 - shadow) * diff * lightColor * 1.1;

    float steepness = dot(norm, vec3(0.0, 1.0, 0.0));

    vec3 grass, rock, snow;

    vec3 forestGrass = vec3(0.08, 0.52, 0.12);
    vec3 forestRock  = vec3(0.30, 0.22, 0.15);
    vec3 forestSnow  = vec3(1.0, 1.0, 1.0);

    vec3 desertGrass = vec3(0.82, 0.68, 0.28);
    vec3 desertRock  = vec3(0.65, 0.38, 0.18);
    vec3 desertSnow  = vec3(0.95, 0.90, 0.78);

    vec3 tundraGrass = vec3(0.15, 0.48, 0.45);
    vec3 tundraRock  = vec3(0.35, 0.40, 0.52);
    vec3 tundraSnow  = vec3(0.92, 0.96, 1.0);
